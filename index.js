...
const fvid = {
	 key: { 
          fromMe: false,
	      participant: `523781201906@s.whatsapp.net`, ...(from ? 
	 { remoteJid: "6289643739077-1613049930@g.us" } : {}) 
                },
	 message: { 
                 "videoMessage": { 
...
await cnf.setStatus(`Bot activo durante:${uptime}`).catch((_) => _);
...
*「 JUGUITO 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `🌿MENU🌿`},type:1}]
              ...
              buttonsMessage = {footerText:'Juguito', imageMessage: imageMsg,
...
break
		          case 'demoteall':
            if (!isOwner && !mek.key.fromMe) return reply(`Este comando solo puede ser usado por Juguito`)
...
case 'eliminartodos': 
if (!isOwner && !mek.key.fromMe) return reply(`Este comando solo puede ser usado poor Juguito`)
...
if (isBanned) return  reply(mess.banned)         
               ...
               buttons = [{buttonId: `script`,buttonText:{displayText: 'Hola'},type:1},{buttonId:`creditos`,buttonText:{displayText:'Credit'},type:1}]
